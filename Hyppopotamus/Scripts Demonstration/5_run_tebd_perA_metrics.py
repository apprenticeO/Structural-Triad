...
def run_perA(L: int, ell: int, J: float, h: float, dt: float, steps: int, chi: int, cut: float, hbar: float,
    ...
    # Recompute from stored data for floors
    for step in range(1, steps + 1, sample_k):
        # This is a simplified version - in practice, you'd store these during the main loop
        # For now, use the final values as proxies
        sqrtF_seq.append(sqrtF)
        S_seq.append(S_A)
        I_seq.append(I_AB)
